...
public class AppFirebaseHelper implements FirebaseHelper {
    ...
    public Flowable<Match> observeScore(String matchId) {
        DatabaseReference databaseReference = firebaseDatabase.getReference().child("matches").child(matchId);
        Log.d(TAG, "observeScore: bude " + matchId);
        return RxFirebaseDatabase.observeValueEvent(databaseReference,Match.class);
    }
    ...
    public void observeScore(String matchId, ChildEventListener listener) {
        firebaseDatabase.getReference().child("matches").child(matchId).addChildEventListener(listener);
        Log.d(TAG, "observeScore: bude " + "executed");
    }
}
